...
В ходе данной работы было проанализировано поведение памяти JVM (**Heap**, **Metaspace**) и динамика загрузки классов в процессе выполнения Java-приложения на основе логов консоли и графиков мониторинга.
...
На графике Heap наблюдаются две ключевые фазы: автоматическая оптимизация и интенсивное наполнение объектами.
...
Эксперимент наглядно подтвердил архитектурные принципы JVM:

1. **Metaspace** выделяется под метаданные классов и растет только при их загрузке, оставаясь стабильным при создании объектов.
2. **Heap** — динамическая область. JVM эффективно управляет её размером: освобождает неиспользуемое и расширяет пространство при массовом создании новых экземпляров объектов.
